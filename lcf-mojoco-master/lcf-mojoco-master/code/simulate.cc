...
int FigureDecimalsFromGridStep(const mjvFigure& fig) {
...
void SetFigureYFormatFromRange(mjvFigure* fig) {
  mju::sprintf_arr(fig->yformat, "%%.%df", FigureDecimalsFromGridStep(*fig));
}

void ApplyFigureTheme(mjvFigure* fig) {
...
void InitializeProfiler(mj::Simulate* sim) {
  ...
  mju::strcpy_arr(sim->figcost.yformat, "%.0f");
  mju::strcpy_arr(sim->figsize.yformat, "%.0f");
  mju::strcpy_arr(sim->figtimer.yformat, "%.3f");

  // colors
  ApplyFigureTheme(&sim->figconstraint);
  ApplyFigureTheme(&sim->figcost);
  ApplyFigureTheme(&sim->figsize);
  ApplyFigureTheme(&sim->figtimer);
  ...
  sim->figconstraint.range[1][1] = 100;
  sim->figcost.range[0][0] = 0;
  sim->figcost.range[0][1] = 20;
  sim->figcost.range[1][0] = -20;
  ...
  sim->figtimer.range[1][1] = 0.5f;

  SetFigureYFormatFromRange(&sim->figcost);
  SetFigureYFormatFromRange(&sim->figtimer);
...
void InitializeSensor(mj::Simulate* sim) {
  ...
  mju::strcpy_arr(figsensor.yformat, "%.2f");
  ...
  SetFigureYFormatFromRange(&figsensor);
}
...
void UpdateInfoText(mj::Simulate* sim,
  ...
  const mjpc::Trajectory* best_trajectory = nullptr;
  int nparam = 0;
  if (sim->agent) {
    best_trajectory = sim->agent->ActivePlanner().BestTrajectory();
    nparam = sim->agent->ActivePlanner().NumParameters();
  }

  char objective_str[64];
  if (best_trajectory) {
    std::snprintf(objective_str, sizeof(objective_str), "%.3f",
                  best_trajectory->total_return);
  } else {
    std::snprintf(objective_str, sizeof(objective_str), "N/A");
  }

  mju::sprintf_arr(content, "%s\n%d\n%d\n%d\n%-9.3f\n%.2g of %s",
                   objective_str, m->nv, m->nu, nparam,
...
void Simulate::Render() {
  ...
  if (this->uiloadrequest.load() == 0) {
    // task-specific
    mjpc::Task* task = this->agent ? this->agent->ActiveTask() : nullptr;
    if (task && task->visualize) {
      task->ModifyScene(this->m, this->d, &this->scn);
    }
    // common to all tasks
    if (this->agent) {
