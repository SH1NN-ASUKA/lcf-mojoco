...
void SetFigureYFormatFromStep(mjvFigure* fig, double step) {
  mju::sprintf_arr(fig->yformat, "%%.%df", DecimalsFromStep(step));
}

void SetFigureYFormatFromRange(mjvFigure* fig) {
  const int grid = fig->gridsize[1] > 0 ? fig->gridsize[1] : 1;
  const double step =
      (static_cast<double>(fig->range[1][1]) - static_cast<double>(fig->range[1][0])) /
      static_cast<double>(grid);
  SetFigureYFormatFromStep(fig, step);
}

void SnapBoundsToNiceGrid(double (&bounds)[2], int grid) {
  grid = std::max(1, grid);
  const double span = bounds[1] - bounds[0];
  if (!(span > 0.0) || !std::isfinite(span)) {
    return;
  }

  const double raw_step = span / static_cast<double>(grid);
  const double step = NiceCeil(raw_step);
  if (!(step > 0.0) || !std::isfinite(step)) {
    return;
  }

  bounds[0] = std::floor(bounds[0] / step) * step;
  bounds[1] = std::ceil(bounds[1] / step) * step;
...
std::string Agent::GetModeName() const {
  if (tasks_.empty() || active_task_id_ < 0 ||
      active_task_id_ >= static_cast<int>(tasks_.size())) {
    return "default_mode";
  }
  ...
  const int mode = ActiveTask()->mode;
  if (mode < 0 || mode >= static_cast<int>(mode_names.size())) {
    return mode_names[0];
  }
...
int Agent::SetModeByName(std::string_view name) {
  if (!model_ || tasks_.empty() || active_task_id_ < 0 ||
      active_task_id_ >= static_cast<int>(tasks_.size())) {
    return -1;
  }
  char* transition_str = GetCustomTextData(model_, "task_transition");
...
void Agent::ModifyScene(mjvScene* scn) {
  if (!scn || !scn->geoms || scn->maxgeom <= 0 || !model_ || tasks_.empty() ||
      active_task_id_ < 0 || active_task_id_ >= static_cast<int>(tasks_.size())) {
    return;
  }
...
void Agent::PlotInitialize() {
  ...
  for (mjvFigure* fig : {&plots_.cost, &plots_.action, &plots_.planner,
                         &plots_.timer}) {
    fig->figurergba[0] = 0.12f;
    fig->figurergba[1] = 0.12f;
    fig->figurergba[2] = 0.12f;
    fig->figurergba[3] = 0.75f;
  }
  ...
  plots_.cost.gridsize[1] = 4;
  plots_.action.gridsize[0] = 3;
  plots_.action.gridsize[1] = 4;
  plots_.planner.gridsize[0] = 3;
  plots_.planner.gridsize[1] = 4;
  plots_.timer.gridsize[0] = 3;
  plots_.timer.gridsize[1] = 4;
...
void Agent::Plots(const mjData* data, int shift) {
  if (!model_ || !data || tasks_.empty() || active_task_id_ < 0 ||
      active_task_id_ >= static_cast<int>(tasks_.size())) {
    return;
  }
  if (allocate_enabled) {
  ...
  SnapBoundsToNiceGrid(cost_bounds, plots_.cost.gridsize[1]);
  ...
  SetFigureYFormatFromRange(&plots_.cost);
  ...
  SetFigureYFormatFromRange(&plots_.action);
  ...
  SetFigureYFormatFromRange(&plots_.planner);
  ...
  timer_bounds[1] = NiceCeil(timer_bounds[1]);
  SnapBoundsToNiceGrid(timer_bounds, plots_.timer.gridsize[1]);

  // legend
  mju::strcpy_arr(plots_.timer.linename[0], "Total");

  // update timer range
  plots_.timer.range[1][0] = timer_bounds[0];
  plots_.timer.range[1][1] = timer_bounds[1];
  SetFigureYFormatFromRange(&plots_.timer);
}
